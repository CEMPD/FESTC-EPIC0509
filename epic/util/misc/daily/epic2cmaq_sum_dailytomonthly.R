...
mrb.file <- paste(outfile,"_MRB.csv",sep="")
for( i in 1:12 ){
   ...
   out.file.1 <- paste(outfile,"_",mm,".csv",sep="")
...
#site file: GRIDID,XLONG,YLAT,ELEVATION,SLOPE_P,HUC8,REG10,STFIPS,CNTYFIPS,GRASS,CROPS,TOTAL,COUNTRY,CNTY_PRO
  ...
  print(out.file.1)
  write.csv( com.df, file = out.file.1 , row.names = FALSE )
  ...
  if ( i == 1 ) mrb.df <- as.data.frame(com.df)
  if ( i > 1 )  mrb.df <- rbind(mrb.df, as.data.frame(com.df))
  str(mrb.df)

}
print(mrb.file)
write.csv( mrb.df, file = mrb.file , row.names = FALSE )

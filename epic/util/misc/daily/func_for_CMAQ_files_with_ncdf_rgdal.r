...
#  require("maps")
  ...
  raw.world.lonlat <- map("worldHires", plot=F, resolution=0, interior = F)
  ## get x and y
  raw.map.x <- c(raw.map.lonlat$x,NA,raw.world.lonlat$x)#append(raw.map.lonlat$x,NA,raw.world.lonlat$x)
  raw.map.y <- c(raw.map.lonlat$y,NA,raw.world.lonlat$y)#append(raw.map.lonlat$y,NA,raw.world.lonlat$y)
#  map.lonlat <- cbind(raw.map.lonlat$x, raw.map.lonlat$y)
  map.lonlat <- cbind(raw.map.x, raw.map.y)
  rm(raw.map.lonlat)

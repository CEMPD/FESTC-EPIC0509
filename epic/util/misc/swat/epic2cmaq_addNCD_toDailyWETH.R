...
for ( i in tlays ) {
  ...
  day.df <- data.frame(GRIDID=daytable$GRIDID,Q=daytable$Q,MUSL=daytable$MUSL,YON=daytable$YON,YP=daytable$YP,QNO3=daytable$QNO3,QAP=daytable$QAP, DATE=daytable$JDATE)
...
cat("m**3, kg/ha, kg/ha, kg/ha, kg/ha, kg/ha\n", file=outfile, append=F)
desc <- paste("Runoff", "Sediment", "N Loss with Sediment","P Loss with Sediment","N Loss in Surface Runoff","Labile P Loss in Runoff", sep=",")

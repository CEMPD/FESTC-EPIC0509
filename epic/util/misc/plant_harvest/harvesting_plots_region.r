...
RegionName=as.vector(c("Region 1", "Region 2","Region 3","Region 4","Region 5","Region 6","Region 7","Region 8","Region 9","Region 10"))
...
print(paste("Layer: ",i, "; Region: ", j, "; spcname:",spcname, sep="") )
x <- start$CROP
...
y <- paste(CropName[i]," - PROGRESS, 5 YEAR AVG, MEASURED IN PCT HARVESTED",sep="")
z <- paste(CropName[i]," - PROGRESS, MEASURED IN PCT HARVESTED",sep="")

epicRE <- subset(sitetable,(sitetable$REG10 %in% j))
if ( length(epicRE$REG10) == 0 ) next

STs <- unique(sort(epicRE$STFIPS))

d<- subset(start,(x%in%y) & (start$StFIPS %in% STs))
dz<- subset(start,(x%in%z) & (start$StFIPS %in% STs))

if ( nrow(d) > 0 ) {
  dd <- data.frame(wk=d$wk, FRAC=d$FRAC)
  ddd <- aggregate(. ~ wk, data=dd, FUN=mean)
}
if ( nrow(dz) > 0 ) {
  ddz <- data.frame(wk=dz$wk, FRAC=dz$FRAC)
  dddz <- aggregate(. ~ wk, data=ddz, FUN=mean)
}
#print(paste("Region:",j," states:",STs, sep="") )

#Define the number of weekly report bins from NASS

if ( nrow(d) > 0 )  week=substr(ddd$wk,7,8)
if ( nrow(dz) > 0 ) weekz=substr(dddz$wk,7,8)
...
epicf <- subset (tem.df, (tem.df$GRIDID %in% epicRE$GRIDID))

#If you do not have both NASS and EPIC data, then do not proceed and go to the end of the loops

print(paste("epic rows:", nrow(epicf),sep="") )
print(paste("NASS rows:", nrow(d),sep="") )
...
if (nrow(d) >0)   cumfrac30=ddd$FRAC
if (nrow(dz) >0)  zcumfrac30=dddz$FRAC
...
plot (fepic$PLANT_CROP,cumarea,main=c("Irrigated",RegionName[j],CropName[i],"Harvest Dates"),col=("red"),pch="",xlab="Week Ending Julian Day",
#ylab="Fraction of Area Plant", ylim=c(ad,ab),xlim=c(ae,ac)) 
 ylab="Fraction of Area Harvest", ylim=c(ad,ab),xlim=c(ae,ac))
...
plot (fepic$PLANT_CROP,cumarea,main=c("Rainfed",RegionName[j],CropName[i],"Harvest Dates"),col=("red"),pch="",xlab="Week Ending Julian Day",
#ylab="Fraction of Area Plant", ylim=c(ad,ab),xlim=c(ae,ac))
 ylab="Fraction of Area Harvest", ylim=c(ad,ab),xlim=c(ae,ac))
lines(fepic$PLANT_CROP,cumarea,col=("red"),lwd=c(2,2))

...
tem.df <- com.df[indic,]
tem.df$HUC2 <- "RSM" 
mrb.df <- aggregate(. ~ HUC2, data=tem.df, FUN=sum)
com.df <- rbind(com.df, mrb.df)

indic  <- acom.df$HUC2 %in% c(5,6,7,8,10,11)
tem.df <- acom.df[indic,]
tem.df$HUC2 <- "RSM" 
amrb.df <- aggregate(. ~ HUC2, data=tem.df, FUN=sum)
acom.df <- rbind(acom.df, amrb.df)

indic  <- pcom.df$HUC2 %in% c(5,6,7,8,10,11)
tem.df <- pcom.df[indic,]
tem.df$HUC2 <- "RSM" 
pmrb.df <- aggregate(. ~ HUC2, data=tem.df, FUN=sum)
pcom.df <- rbind(pcom.df, pmrb.df)
}
